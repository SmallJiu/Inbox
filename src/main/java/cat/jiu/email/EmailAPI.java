...
public class EmailAPI {
	...
	public static synchronized boolean sendEmail(EmailSenderGroup group, String address, Email email) {
		...
		if(NeoForge.EVENT_BUS.post(new EmailSendEvent(true, group, address, email)).isCanceled()) return false;
		
		inbox.addEmail(email, true);

		NeoForge.EVENT_BUS.post(new EmailSendEvent(false, group, address, email));
