...
public interface IAttachment extends Consumer<Player>, ISerializable, Supplier<ResourceLocation> {
    ...
    ResourceLocation EMPTY_ID = ResourceLocation.fromNamespaceAndPath(EmailMain.MODID, "attachment/empty");
    String ID_NAME = "id";
    ...
    DynamicRegistry<ResourceLocation, IAttachment> REGISTRY = new DynamicRegistry<ResourceLocation, IAttachment>((id, json) -> {
        ...
        return EMPTY;
            ...
            data->ResourceLocation.parse(data.getString(ID_NAME)),
            data->ResourceLocation.parse(data.get(ID_NAME).getAsString())
    );
