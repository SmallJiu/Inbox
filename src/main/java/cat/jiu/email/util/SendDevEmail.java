	...
	public static void onPlayerTick(TickEvent.PlayerTickEvent event){
			...
			if(delay.msg >= 10 * 20){
				...
				if(un.unread>0 && un.unReceived>0){
					player.displayClientMessage(new TranslatableComponent("info.email.has_unread_and_unreceive", un.unread, un.unReceived), true);
				} else if (un.unread > 0) {
					player.displayClientMessage(new TranslatableComponent("info.email.has_unread", un.unread), true);
				} else if (un.unReceived > 0) {
					player.displayClientMessage(new TranslatableComponent("info.email.has_unreceive", un.unReceived), true);
				}
			}else {
				delay.msg++;
			}
