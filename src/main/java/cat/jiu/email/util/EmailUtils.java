...
public class EmailUtils {
	...
	public static void sendMessage(Player player, ChatFormatting color, String key, Object... args) {
		player.sendMessage(new Text(key, formatArgsTextToComponent(args)).toTextComponent(color), player.getUUID());
	}
	
	public static void sendMessage(Player player, String key, Object... args) {
		player.sendMessage(new Text(key, formatArgsTextToComponent(args)).toTextComponent(), player.getUUID());
	}
	...
	public static Component createTextComponent(String arg, Object... objs) {
		return new TranslatableComponent(arg, objs);
	}
	
	public static Component createTextComponent(ChatFormatting color, String arg, Object... objs) {
		Component text = new TranslatableComponent(arg, objs);
	...
	public static long getSize(CompoundTag nbt) {
		...
		NbtAccounter tracker = new NbtAccounter(0) {
			@Override
			public void accountBits(long pBytes) {
				this.usage += pBytes;
			}
		};
...
//		}
	...
	public static final File USERID_CACHE_FILE = MinecraftServer.USERID_CACHE_FILE;
	...
	@OnlyIn(Dist.CLIENT)
    public static void drawAlignRightString(PoseStack stack, String text, int x, int y, int color, boolean drawShadow) {
        drawAlignRightString(stack, Minecraft.getInstance().font, text, x, y, color, drawShadow);
    }

	@OnlyIn(Dist.CLIENT)
    public static void drawAlignRightString(PoseStack stack, Font fr, String text, int x, int y, int color, boolean drawShadow) {
		if (drawShadow) {
			fr.drawShadow(stack, text, x - fr.width(text), y, color);
		}else {
			fr.draw(stack, text, x - fr.width(text), y, color);
		}
    }
	@OnlyIn(Dist.CLIENT)
	public static void drawString(PoseStack stack, String text, int x, int y, int color, boolean drawShadow) {
		drawString(stack, Minecraft.getInstance().font, text, x, y, color, drawShadow);
	}

	@OnlyIn(Dist.CLIENT)
	public static void drawString(PoseStack stack, Font fr, String text, int x, int y, int color, boolean drawShadow) {
		if (drawShadow) {
			fr.drawShadow(stack, text, x, y, color);
		}else {
			fr.draw(stack, text, x, y, color);
		}
	}
