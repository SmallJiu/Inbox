...
public final class JsonToStackUtil {
	...
	public static JsonObject toJson(ItemStack arg) {
		...
		obj.addProperty("name", Registry.ITEM.getKey(arg.getItem()).toString());
	...
	private static String toString(ItemStack arg) {
		...
		j.add(Registry.ITEM.getKey(arg.getItem()).toString());
	...
	private static ItemStack toStack(String stack) {
		if(stack.contains("@")) {
			...
			Item item = Registry.ITEM.get(new ResourceLocation(name[0]));
			if(item!=null) {
				int amount = 1;
			...
			return new ItemStack(Registry.ITEM.get(new ResourceLocation(stack)));
		}
		return null;
	}
