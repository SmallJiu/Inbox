...
public final class Inbox implements ISerializable {
	...
	private Inbox(String owner, CompoundNBT inboxTag) {
		this.owner = owner;
		this.read(inboxTag);
	}
	...
	public long getInboxSize() {
		CompoundNBT nbt = this.writeTo(CompoundNBT.class);
	...
	private Inbox readFromDisk(CompoundNBT nbt) {
		this.emails.clear();
		this.customValue.clear();
		this.senderBlacklist.clear();
		this.dev = false;
...
//		EmailMain.log.error(String.valueOf(json));
	...
	public CompoundNBT write(CompoundNBT nbt) {
		...
		if(!this.isEmptyInbox()) {
			CompoundNBT emails = new CompoundNBT();
			for(Entry<Long, Email> email : this.emails.entrySet()) {
				emails.put(String.valueOf(email.getKey()), email.getValue().writeTo(CompoundNBT.class));
			}
	...
	public void read(CompoundNBT nbt) {
		if(nbt!=null && !nbt.isEmpty()) {
			...
			if(nbt.contains("custom")) {
				CompoundNBT customTag = nbt.getCompound("custom");
				for(String custom : customTag.keySet()) {
					INBT customValue = customTag.get(custom);
					if(customValue instanceof ByteNBT) {
						this.customValue.put(custom, ((ByteNBT)customValue).getByte()==1);
					}else if(customValue instanceof IntNBT) {
						this.customValue.put(custom, ((IntNBT)customValue).getInt());
					}else {
						this.customValue.put(custom, customValue.getString());
					}
				}
			}
			
			if(nbt.contains("emails")) {
				CompoundNBT emailTag = nbt.getCompound("emails");
				if(!emailTag.isEmpty()) {
					for(String emailKey : emailTag.keySet()) {
	 ...
	 */
	public static Inbox get(@Nonnull PlayerEntity player) {
	 ...
	 * @return the inbox
	 */
	public static Inbox get(@Nonnull UUID uid, CompoundNBT inboxTag) {
