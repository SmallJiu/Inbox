		...
		if(json!=null && json.size()>0) {
			...
			if (json.has("attachments")) {
				for (JsonElement element : json.getAsJsonArray("attachments")) {
					...
					this.addAttachment(IAttachment.REGISTRY.get(ResourceLocation.parse(object.get("id").getAsString()), object));
				}
			}
		...
		if(nbt!=null && !nbt.isEmpty()) {
			...
			if(nbt.contains("items")) {
				...
				for(String item : items.getAllKeys()) {
					ItemStack.CODEC.parse(NbtOps.INSTANCE, items.getCompound(item)).ifSuccess(this::addItem);
				}
			}
