...
public class Email implements ISerializable {
	...
	protected AudioSystem.Audio network_local_sound;
	 ...
	 * @author small_jiu
	 */
	public Email(@Nonnull IText title, @Nonnull IText sender, AudioSystem.Audio sound, List<ItemStack> items, List<IText> messages) {
		this.title = title;
		this.sender = sender;
		this.network_local_sound = sound;
	...
	/**
	 *
	 * @param network_local_sound 本地或网络音效
	 */
	public Email setNetworkLocalSound(AudioSystem.Audio network_local_sound) {
		this.network_local_sound = network_local_sound;
		this.setIsNetworkOrLocalSound(true);
		return this;
	}

	/**
	 *
	 * @param networkOrLocalSound 是否是本地或网络音效
	 */
	public Email setIsNetworkOrLocalSound(boolean networkOrLocalSound) {
		isNetworkOrLocalSound = networkOrLocalSound;
		return this;
	}
	...
	public boolean hasSound() {
		return (this.mc_sound !=null && this.mc_sound.getSound()!=null) || (this.network_local_sound != null && !this.network_local_sound.file().isEmpty());
	}
	/**
	 * @return 是否带有附加的物品
	 */
	 ...
	 */
	public Email copy() {
		Email copy = new Email(this.getTitle().copy(), this.getSender().copy(), (ISound) null, null, null);
		if(this.hasSound()) {
			if (this.isNetworkOrLocalSound()) {
				copy.network_local_sound = this.network_local_sound;
			}else {
	...
	public JsonObject write(JsonObject json) {
		...
		if(this.hasSound()) {
			if (this.isNetworkOrLocalSound()) {
				...
				sound_obj.addProperty("file", this.getNetworkOrLocalSound().file());
				sound_obj.addProperty("category", this.getNetworkOrLocalSound().category().getName());
				json.add("sound", sound_obj);
				json.addProperty("network_local_sound", true);
			}else {
		...
		if(json!=null && json.size()>0) {
			...
			if(json.has("sound")) {
				if (json.has("network_local_sound") && json.get("network_local_sound").getAsBoolean()) {
					...
					this
							.setNetworkLocalSound(new AudioSystem.Audio(
	...
	public CompoundTag write(CompoundTag nbt) {
		...
		if(this.hasSound()) {
			if (this.isNetworkOrLocalSound()) {
				...
				sound_obj.putString("file", this.getNetworkOrLocalSound().file());
				sound_obj.putString("category", this.getNetworkOrLocalSound().category().getName());
				nbt.put("sound", sound_obj);
				nbt.putBoolean("network_local_sound", true);
			}else {
	...
	public void read(CompoundTag nbt) {
		if(nbt!=null && nbt.size()>0) {
			...
			if(nbt.contains("sound")) {
				if (nbt.contains("network_local_sound") && nbt.getBoolean("network_local_sound")) {
			...
			if(nbt.contains("items")) {
				...
				for(String item : items.getAllKeys()) {
					this.items.add(ItemStack.of(items.getCompound(item)));
				}
			}
			if(nbt.contains("msgs")) {
				...
				keys.forEach(key->{
					...
					}else if(msg instanceof CompoundTag) {
						CompoundTag text = (CompoundTag) msg;
	...
	public int hashCode() {
		...
		result = prime * result + ((mc_sound == null) ? 0 : mc_sound.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if(sender == null) {
			...
			return false;
		if(mc_sound == null) {
			if(other.mc_sound != null)
				return false;
		}else if(!mc_sound.equals(other.mc_sound))
			return false;
		if(title == null) {
