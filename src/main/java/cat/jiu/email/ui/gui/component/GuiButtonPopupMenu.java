...
public class GuiButtonPopupMenu extends Screen {
    ...
    public void drawPopupMenu(PoseStack stack, int x, int y, float partialTicks) {
        if (this.isVisible()) {
            int btnY = this.createY;
...
//            graphics.hLine(btn1.getX(), btn1.getX() + btn1.getWidth() - 1, btn1.getY() + btn1.getHeight()-1, Color.BLACK.getRGB());
            ...
            for (int i : this.scroll.getShows()) {
                ...
                if (isInRange(x,y, btn.x, btn.y, btn.getWidth(), btn.getHeight())) {
                    hLine(stack, btn.x, btn.x+btn.getWidth()-1, btn.y+btn.getHeight()-1, Color.WHITE.getRGB());
                }
            }
        }
    }
