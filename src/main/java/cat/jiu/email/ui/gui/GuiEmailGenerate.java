...
public class GuiEmailGenerate extends AbstractContainerScreen<ContainerEmailGenerate> {
	public static final ResourceLocation BackGround = ResourceLocation.fromNamespaceAndPath(EmailMain.MODID, "textures/gui/container/inbox_generate.png");
    public static final WidgetSprites SEND_INV_ITEMS_ICON = new WidgetSprites(ResourceLocation.withDefaultNamespace("icon/draft_report"), ResourceLocation.withDefaultNamespace("icon/draft_report"));
	...
	public void init() {
		...
		this.useMCSound = this.addRenderableWidget(Checkbox.builder(CommonComponents.EMPTY, Minecraft.getInstance().font)
				.pos(this.localSound.getX() + this.localSound.getWidth() + 2, this.localSound.getY())
				.maxWidth(12)
				...
				.tooltip(Tooltip.create(Component.translatable("info.inbox.generate.sound.use_mc_sound")))
				.build());
		if (EmailUtils.isOP(Minecraft.getInstance().player)) {
			...
			Button saveItemsBtn = this.addRenderableWidget(new ImageButton(
					...
					15, 12, SEND_INV_ITEMS_ICON, button -> {
				if (Screen.hasShiftDown()) {
					if (this.stacks!=null) this.stacks.clear();
					button.setTooltip(Tooltip.create(info));
					return;
				}
				if (this.stacks==null) {
		...
		if(!this.getMenu().isLock()) {
			...
			if (this.useMCSound.selected()) {
				email.setMcSound(new SoundMC()
						...
						.setSoundEvent(BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse(this.mcSoundBtn.getMessage().getString())))
	...
	public boolean mouseScrolled(double pMouseX, double pMouseY, double scrollX, double scrollY) {
		if (this.mcSounds.scroll((int) pMouseX, (int) pMouseY, (int) scrollX)) {
			return true;
		}
