...
public abstract class NumberEntry<T extends Number> extends ConfigEntry<T> {
    ...
    public void render(Screen gui, PoseStack stack, int x, int y, int mouseX, int mouseY) {
        ...
        this.drawAlignRightString(stack, this.configName, this.field.x - 5, this.field.y + 5, Color.WHITE.getRGB(), true, gui.getMinecraft().font);
    }

    @Override
    public void drawHoverText(Screen gui, PoseStack stack, int mouseX, int mouseY) {
        try {
            this.drawCommentWithRange(gui, stack, mouseX, mouseY,
                    this.field.x -5-gui.getMinecraft().font.width(this.configName), this.field.y +5,
