...
public class MsgSend extends BaseMessage {
	public static final Type<MsgSend> TYPE = type(EmailMain.MODID);
	...
	private void sendEmail(ServerPlayer sender, String addresses, boolean lock) {
		...
		if(NeoForge.EVENT_BUS.post(new EmailSendEvent(true, this.group, addresses, this.email)).isCanceled()) return;
		...
		NeoForge.EVENT_BUS.post(new EmailSendEvent(false, this.group, addresses, this.email));
	}
