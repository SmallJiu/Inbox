...
public class MsgReadEmail extends BaseMessage {
	public static final Type<MsgReadEmail> TYPE = type(EmailMain.MODID);
...
//						EmailAPI.sendInboxToClient(inbox, player);
                    ...
                    NeoForge.EVENT_BUS.post(new EmailReadEvent.Post(player, inbox, email, false));
                }
            }
        });
        return true;
		...
		public boolean handler(IPayloadContext ctx) {
            ctx.enqueueWork(() -> {
                ...
                for (long i : inbox.getEmailIDs()) {
                    ...
                    NeoForge.EVENT_BUS.post(new EmailReadEvent.Post(player, inbox, email, true));
                }
