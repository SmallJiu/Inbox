			...
			if(ctx.get().getSender() != null) {
				ctx.get().enqueueWork(()->{
					...
					if(!inbox.isInSenderBlacklist(this.name)) {
						...
						inbox.saveToDisk();
						ctx.get().getSender().displayClientMessage(EmailUtils.createTextComponent(ChatFormatting.GREEN, "info.email.black.add.success", name), false);
		...
		public boolean handler(Supplier<NetworkEvent.Context> ctx) {
			if(ctx.get().getSender() != null) {
				ctx.get().enqueueWork(()->{
					...
					ctx.get().getSender().displayClientMessage(EmailUtils.createTextComponent(ChatFormatting.GREEN, "info.email.black.remove.success", this.name), false);
				});
			}
			return true;
		}
