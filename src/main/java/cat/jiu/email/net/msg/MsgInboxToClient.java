		...
		if(EmailMain.proxy.isClient()) {
			...
			if(con instanceof ContainerEmailMain) {
				if(!SizeReport.SUCCESS.equals(this.report)) {
					player.displayClientMessage(Component.nullToEmpty(ChatFormatting.GRAY + "---------------------------------------------"), false);
					player.displayClientMessage(new TranslatableComponent("info.email.error.to_big.0"), false);
					player.displayClientMessage(new TranslatableComponent("info.email.error.to_big.1", this.report.id(), this.report.slot(), this.report.size()), false);
