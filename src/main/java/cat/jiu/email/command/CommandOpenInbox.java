            ...
            if (ctx.getSource().hasPermission(4)) {
                for (GameProfile profile : profiles) {
                    ...
                    if (player1 != null) {
                        ...
                        ctx.getSource().sendFailure(new TranslatableComponent(ChatFormatting.RED + String.format("找不到在线玩家：%s", profile.getName())));
                    }
                }
            }else {
                ctx.getSource().sendFailure(new TranslatableComponent(ChatFormatting.RED + "你没有所需权限。"));
            }
