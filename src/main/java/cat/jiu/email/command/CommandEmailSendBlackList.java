...
class CommandEmailSendBlackList extends CommandTreeBase {
	static final TextComponentTranslation black = new TextComponentTranslation("email.command.send.list.black");
	...
	class Add extends CommandBase {
		...
		public String getUsage(ICommandSender sender) {return "email.command.send.black.add";}
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
			if(!EmailConfigs.Send.Enable_Send_BlackList) {
				throw new CommandException("email.command.send.list.unenable", black);
			}
			...
			if(name != null && uid != null) {
				if(Email.isInBlackList(name) || Email.isInBlackList(uid)) {
					...
					if(lag) {
						...
						throw new CommandException("email.command.send.list.error");
					}
				}
			}else {
				throw new CommandException("email.command.send.list.not_found_player", argName, black);
			}
		}
	...
	class Remove extends CommandBase {
		...
		public String getUsage(ICommandSender sender) {return "email.command.send.black.remove";}
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
			if(!EmailConfigs.Send.Enable_Send_BlackList) {
				throw new CommandException("email.command.send.list.unenable", black);
			}
