    ...
    public int execute(MinecraftServer server, CommandSource sender, String[] args, CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        if (sender instanceof Player) {
            if (args.length == 0) {
                ...
                ctx.getSource().sendSuccess(new TranslatableComponent(String.format("已导出主副手物品至 %s, 耗时 %s毫秒", path, System.currentTimeMillis() - time)), false);
            } else {
                long time = System.currentTimeMillis();
                ...
                JsonParser.toJsonFile(path, stacks, false);
                ctx.getSource().sendSuccess(new TranslatableComponent(String.format("已导出背包物品至 %s, 耗时 %s 毫秒", path, System.currentTimeMillis() -time)), false);
            }
        }
        return 1;
    }
