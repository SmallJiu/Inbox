    ...
    public int execute(MinecraftServer server, CommandSource sender, String[] args, CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if(!inbox.isEmptyInbox()) {
            if(inbox.hasEmail(email)){
                ...
                ctx.getSource().sendSuccess(new TranslatableComponent(String.format(ChatFormatting.GREEN + "已删除 %s 的邮箱中ID为 %s 的邮件.", player, email)), false);
            }else {
                ctx.getSource().sendFailure(new TranslatableComponent(String.format(ChatFormatting.YELLOW + "在 %s 的邮箱中找不到ID为 %s 的邮件.", player, email)));
            }
        }else {
            ctx.getSource().sendFailure(new TranslatableComponent(ChatFormatting.RED + String.format("无法找到 '%s' 的邮箱", id)));
        }
        return 1;
    }
}
