...
public final class JsonToStackUtil {
	...
	public static JsonObject toJson(ItemStack arg) {
		...
		obj.addProperty("damage", arg.getDamage());
		if(arg.hasTag()) {
	...
	private static String toString(ItemStack arg) {
		...
		j.add(Integer.toString(arg.getDamage()));
		if(arg.hasTag()) {
	...
	public static JsonArray toJson(ListNBT list) {
		JsonArray array = new JsonArray();
		for (INBT inbt : list) {
	...
	public static JsonObject toJson(CompoundNBT nbt) {
		JsonObject obj = new JsonObject();
	...
	private static void addNum(JsonObject o, String k, String k1, JsonPrimitive primitive) {
		if(!o.has(k)) {
			o.add(k, new JsonObject());
		}
		o.get(k).getAsJsonObject().add(k1, primitive);
	}
	
	private static JsonElement toJson(INBT base) {
		if(base instanceof NumberNBT) {
			return new JsonPrimitive(((NumberNBT) base).getAsNumber());
		}else if(base instanceof StringNBT) {
			JsonArray num_array = getNumberArray((StringNBT) base);
			if(num_array != null) return num_array;
			
			return new JsonPrimitive(base.getString());
		}else if(base instanceof CompoundNBT) {
			return toJson((CompoundNBT) base);
		}else if(base instanceof ListNBT) {
			return toJson((ListNBT) base);
		}else if(base instanceof IntArrayNBT) {
			JsonArray array = new JsonArray();
			for(int i : ((IntArrayNBT)base).getIntArray()) {
				array.add(i);
			}
			return array;
		}else if(base instanceof ByteArrayNBT) {
			JsonArray array = new JsonArray();
			for(byte i : ((ByteArrayNBT)base).getByteArray()) {
				array.add(i);
			}
			return array;
		}
	...
	private static JsonArray getNumberArray(StringNBT str) {
	...
	private static ItemStack toStack(JsonObject obj) {
		...
		CompoundNBT nbt = obj.has("nbt") ? toNBT(obj.get("nbt").getAsJsonObject()) : null;

		return setNBT(new ItemStack(item, count), nbt);
	}
	
	public static CompoundNBT toNBT(JsonObject obj) {
	...
	public static ListNBT toNBT(JsonArray array) {
		...
		for(int i = 0; i < array.size(); i++) {
			...
			if(array_element.isJsonPrimitive()) {
				...
				if(pri.isString()) {
					list.add(StringNBT.valueOf(pri.getAsString()));
					continue;
				}
				if(pri.isBoolean()) {
					list.add(ByteNBT.valueOf(pri.getAsBoolean() ? (byte)1 : (byte)0));
					continue;
				}
				if(pri.isNumber()) {
					Number num = pri.getAsNumber();
					if(num instanceof Integer) {
						list.add(IntNBT.valueOf((Integer) num));
						continue;
					}
					if(num instanceof Double) {
						list.add(DoubleNBT.valueOf((Double) num));
						continue;
					}
					if(num instanceof Byte) {
						list.add(ByteNBT.valueOf((Byte) num));
						continue;
					}
					if(num instanceof Long) {
						list.add(LongNBT.valueOf((Long) num));
						continue;
					}
					if(num instanceof Float) {
						list.add(FloatNBT.valueOf((Float) num));
						continue;
					}
					if(num instanceof Short) {
						list.add(ShortNBT.valueOf((Short) num));
					}
				}
			}
		}
	...
	private static ItemStack toStack(String stack) {
		if(stack.contains("@")) {
			...
			Item item = Registry.ITEM.getOrDefault(new ResourceLocation(name[0]));
			if(item!=null) {
				int amount = 1;
			...
			return new ItemStack(Registry.ITEM.getOrDefault(new ResourceLocation(stack)));
		}
		return null;
	}
	
	private static CompoundNBT setNBT(CompoundNBT nbt, String nbtName, double[] value) {
		nbt.putString(nbtName, "double_array@" + toString(toArray(value)));
		return nbt;
	}
	private static CompoundNBT setNBT(CompoundNBT nbt, String nbtName, float[] value) {
		nbt.putString(nbtName, "float_array@" + toString(toArray(value)));
		return nbt;
	}
	private static CompoundNBT setNBT(CompoundNBT nbt, String nbtName, short[] value) {
		nbt.putString(nbtName, "short_array@" + toString(toArray(value)));
		return nbt;
	}
	
	private static ItemStack setNBT(ItemStack stack, CompoundNBT nbt) {
