...
public class DynamicRegistry<K, V extends IJsonSerializable & INBTSerializable & Supplier<K>> {
    ...
    public static <K, V> FailBack<K, CompoundNBT, V> nbtFailBack(ResourceLocation typeID) {
        return (id, data)->{
            LogManager.getLogger("Registry").fatal("{} is not register to {}.", id, typeID);
            return null;
        };
    ...
    protected final FailBack<K, CompoundNBT, V> nbtFailBack;
    protected Function<CompoundNBT, K> nbtKeyGetter;
    ...
    public DynamicRegistry(
            ...
            FailBack<K, CompoundNBT, V> nbtFailBack
    ) {
    ...
    public DynamicRegistry<K, V> setKeyGetter(Function<CompoundNBT, K> nbtKeyGetter, Function<JsonObject, K> jsontKeyGetter) {
    ...
    public K register(K id, Function<CompoundNBT, V> nbtGetter, Function<JsonObject, V> jsonGetter) {
    ...
    public V get(K id, CompoundNBT tag) {
        if (this.registry.containsKey(id)) {
    ...
    protected static class Getter<T extends IJsonSerializable & INBTSerializable> {
        protected final Function<CompoundNBT, T> nbtGetter;
        protected final Function<JsonObject, T> jsonGetter;

        public Getter(Function<CompoundNBT, T> nbtGetter, Function<JsonObject, T> jsonGetter) {
