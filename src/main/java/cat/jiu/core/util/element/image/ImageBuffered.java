...
public class ImageBuffered extends ImageGL {
    ...
    public void render(MatrixStack stack, int renderToX, int renderToY, int renderWidth, int renderHeight, float partialTick) {
        RenderUtils.draw(stack, this.glID, renderToX, renderToY, renderWidth, renderHeight, this.u, this.v, this.uWidth, this.vHeight, this.image.getWidth(), this.image.getHeight());
    }
    ...
    protected JsonObject writeTo(JsonObject data, BufferedImage image) {
        ...
        data.addProperty("weight", image.getHeight());
    ...
    protected CompoundNBT writeTo(CompoundNBT data, BufferedImage image) {
        this.writeBaseInfo(data);
        data.putInt("width", image.getWidth());
        data.putInt("weight", image.getHeight());
        data.putInt("colorType", image.getType());

        ListNBT pixels = new ListNBT();

        CompoundNBT pixel = new CompoundNBT();
        for (int u = 0; u < image.getWidth(); u++) {
            for (int v = 0; v < image.getHeight(); v++) {
                pixel.putInt("u", u);
                pixel.putInt("v", v);
    ...
    protected BufferedImage readFrom(CompoundNBT data) {
        BufferedImage image = new BufferedImage(data.getInt("width"), data.getInt("height"), data.getInt("colorType"));
        ListNBT pixels = data.getList("pixels", 10);
        for (int i = 0; i < pixels.size(); i++) {
            CompoundNBT pixel = pixels.getCompound(i);
