...
public class ImageJarPath extends BaseGifImage {
    ...
    public CompoundNBT write(CompoundNBT data) {
        ...
        this.paths.forEach((clazz, list)->{
            ...
            ListNBT array1 = new ListNBT();
            list.forEach(path->array1.add(StringNBT.valueOf(path)));
    ...
    public void read(CompoundNBT data) {
        this.readBaseInfo(data);
        ListNBT array = data.getList("paths", 10);
        for (int i = 0; i < array.size(); i++) {
            CompoundNBT object = array.getCompound(i);
            try {
                Class<?> clazz = Class.forName(object.getString("clazz"));
                ListNBT array1 = data.getList("path", 8);
