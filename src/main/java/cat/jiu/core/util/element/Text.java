...
public class Text implements IText {
	...
	public Text copy() {
		return new Text(this.writeTo(CompoundNBT.class));
	}
	...
	public ListNBT writeArgs(ListNBT args) {
		if(args==null) args = new ListNBT();
		if(this.args!=null&&this.args.length>0) {
			for(int j = 0; j < this.args.length; j++) {
				args.add(StringNBT.valueOf(String.valueOf(this.args[j])));
			}
		}
		return args;
	}
