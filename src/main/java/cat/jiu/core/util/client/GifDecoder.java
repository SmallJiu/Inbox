...
public class GifDecoder {
	 ...
	 * @author small_jiu
	 */
	public static interface IGifTexture extends IRenderable {
		@OnlyIn(Dist.CLIENT)
		default void render(MatrixStack stack) {
			this.render(stack, -1, true);
		}
		...
		@OnlyIn(Dist.CLIENT)
		void render(MatrixStack stack, long delay, boolean addCurrentIndex);
...
//			EmailMain.log.info("{} write full png image to '{}'",  res ? "Success" : "Fail",file);
	 ...
	 * @author small_jiu
	 */
	public static class BufferedTexture extends Texture {
...
//				this.deleteGlTexture();
	 ...
	 */
	@OnlyIn(Dist.CLIENT)
	public static class MCGifTexture extends Texture {
		protected final IGifTexture gif;
		public MCGifTexture(IGifTexture gif) {
			this.gif = gif;
	...
	public static class SingletonGifTexture implements IGifTexture {
		...
		public ResourceLocation registerToMinecraft(ResourceLocation id) {
			if (this.mcPath == null) {
				this.mcPath = id;
				Minecraft.getInstance().getTextureManager().loadTexture(id, new MCGifTexture(this));
			}
			return id;
		}
		...
		@OnlyIn(Dist.CLIENT)
		@Override
		public void render(MatrixStack stack, long delay, boolean addCurrentIndex) {
			GifTexture.draw(stack,
					this.glID,
		...
		@Override
		public void destroy() {
			GlStateManager.deleteTexture(this.glID);
		}
...
//			graphics.drawString(Minecraft.getInstance().font, String.valueOf(this.getCurrentRenderIndex()), this.x, this.y - 20, Color.RED.getRGB(), true);
		...
		public static void draw(MatrixStack stack, int texture, int x, int y, int width, int height, float u, float v, int uWidth, int vHeight, float textureWidth, float textureHeight) {
			int
					x2 = x + width,
					y2 = y + height;
			float
...
//			BufferUploader.drawWithShader(bufferbuilder.end());
			...
			bufferbuilder.pos(stack4f, (float)x, (float)y2, (float)blitOffset).tex(minU, maxV).endVertex();
			bufferbuilder.pos(stack4f, (float)x2, (float)y2, (float)blitOffset).tex(maxU, maxV).endVertex();
			bufferbuilder.pos(stack4f, (float)x2, (float)y, (float)blitOffset).tex(maxU, minV).endVertex();
			bufferbuilder.pos(stack4f, (float)x, (float)y, (float)blitOffset).tex(minU, minV).endVertex();
...
//					EmailMain.log.fatal("Write full png image is fail, index: '{}', path: {}", i, f1);
		...
		public void render(MatrixStack stack, long delay, boolean addCurrentIndex) {
			this.nextFrame(delay, addCurrentIndex);
			this.textures.get(this.getCurrentImageIndex()).render(stack, delay, addCurrentIndex);
		}
