...
public interface ISound1 extends ISerializable {
	...
	ISound1 setSoundCategory(SoundCategory sc);
	...
	default CompoundNBT write(CompoundNBT nbt) {
		...
		nbt.putString("sound", ForgeRegistries.SOUND_EVENTS.getKey(this.getSound()).toString());
	...
	default void read(CompoundNBT nbt) {
		...
		this.setSound(ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(nbt.getString("sound"))));
	...
	default JsonObject write(JsonObject json) {
		...
		json.addProperty("id", ForgeRegistries.SOUND_EVENTS.getKey(this.getSound()).toString());
	...
	default void read(JsonObject json) {
		...
		if(json.getAsJsonPrimitive("id").isString()) {
			sound = ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(json.get("id").getAsString()));
		}
//		else if(json.getAsJsonPrimitive("id").isNumber()) {
//			sound = ForgeRegistries.SOUND_EVENTS.(json.get("id").getAsInt()).get().get();
//		}
		this.setSound(sound);
	...
	static CompoundNBT writePositionNBT(BlockPos pos) {
	...
	static BlockPos readPosition(CompoundNBT nbt) {
