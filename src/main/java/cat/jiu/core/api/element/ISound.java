...
public interface ISound extends ISerializable {
	...
	default void read(CompoundTag nbt) {
		...
		this.setSound(Registry.SOUND_EVENT.get(new ResourceLocation(nbt.getString("sound"))));
	...
	default void read(JsonObject json) {
		...
		if(json.getAsJsonPrimitive("id").isString()) {
			sound = Registry.SOUND_EVENT.get(new ResourceLocation(json.get("id").getAsString()));
		}else if(json.getAsJsonPrimitive("id").isNumber()) {
			sound = Registry.SOUND_EVENT.getHolder(json.get("id").getAsInt()).get().value();
		}
		this.setSound(sound);
