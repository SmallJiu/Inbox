...
public class MultiplexAudio extends RandomRead {
	...
	private boolean start_code() throws IOException {
		...
		//if(idx > 0)
		//	jmp123.Jmp123Logger.info("skip bytes:" + idx);
		return true;
	}
	...
	private void pack() throws IOException {
		...
		if((buf[curPos+4] & 0xf0) == 0x20) {		// '0010 xxxx'
			...
			if (this.duration == 0) {
				...
				jmp123.Jmp123Logger.info("duration: %ds\n", duration);
			}
	...
	private boolean audio_stream() throws IOException {
		...
		while(true) {
			...
			if((startcode = buf[curPos+3] & 0xff) < 0xb9) {
				//jmp123.Jmp123Logger.info("Not system stream. stream_id=" + startcode + ", skip...");
				curPos += 4;
				continue;
			}
			...
			case 0xc0:	// '110x xxxx': audio stream
				...
				if(packet_data_bytes > 2324) {
					//jmp123.Jmp123Logger.info("packet_data_byte_len=" + packet_data_bytes + " skip...");
					curPos += 4;
					break;
				}
			...
			default:
				...
				if(header_length > 2324) {
					//jmp123.Jmp123Logger.info("header_length = " + header_length + " skip...");
					curPos += 4;
				} else
	...
	public void close() {
		try {
			this.raf.close();
			//jmp123.Jmp123Logger.info("audiopacket_data_bytes="+audiopacket_data_bytes);
		} catch (IOException e) {}
	}
