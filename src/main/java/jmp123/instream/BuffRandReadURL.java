...
public class BuffRandReadURL extends RandomRead {
	...
	private final byte[] lock; // 读和写(缓冲)互斥
	private final byte[] buf;  // 同时作写线程同步锁
	...
	public boolean open(String spec, String title) throws IOException {
		...
		s1 = URLDecoder.decode(s1, StandardCharsets.UTF_8.name());
...
//		jmp123.Jmp123Logger.info("\nPLAY>> " + s1);

		acceptRanges = "bytes".equals(connection.getHeaderField("Accept-Ranges"));
		//if(!acceptRanges)
		//	jmp123.Jmp123Logger.info(url.getHost() + ": not support multi-threaded downloads.");
	...
	private class Writer extends Thread {
		...
		public void run() {
			...
			try {
				while (!eof) {
					...
					try {
						while (rema > 0 && !eof) {
							...
							//jmp123.Jmp123Logger.infof("bytes=%,d  len=%d  rema=%d\n", bytes, len, rema);
						}
					} catch (SocketTimeoutException e) {
						retry++;
						jmp123.Jmp123Logger.info("[B# %,d] Timeout, retry=%d\n",bytes, retry);
						if (retry < 10)
							continue;
						jmp123.Jmp123Logger.info("B# %,d: out of retries. Giving up.\n", bytes);
						eof = true; // 终止下载
					}
					retry = 0;
				...
				jmp123.Jmp123Logger.info("BuffRandReadURL.Writer.run(): " + e);
			} finally {
				eof = true;
				synchronized (lock) {
					lock.notify();
