...
//		if(skipBytes > 0)
//			jmp123.Jmp123Logger.infof("frame# %d, skip bytes: %d\n",framecounter, skipBytes);
	...
	private void parseVBR(byte[] b, int off) {
		...
		for (int i = 2; i < sideinfosize; i++)	//前2字节可能是CRC_word
			if (b[off + i] != 0)
				return;
		off += sideinfosize;
		//jmp123.Jmp123Logger.info("tagsize=" + (frameSize-4-sideinfoSize));
		...
		*/
		...
		if(mp3_gain != 0)
			jmp123.Jmp123Logger.info("    MP3 Gain: "+mp3_gain+" [psa="+peak+",rrg="+radio+",arg="+phile+"]");
	...
	private int xinginfoHeader(byte[] b, int off) {
		...
		if ((xing_flags & 0x4) != 0) { // TOC: 100-byte
			...
			off += 100;
			//jmp123.Jmp123Logger.info("         TOC: yes");
		}
...
//		jmp123.Jmp123Logger.info(info.toString());
	}
	...
	public void printVBRTag() {
		if(vbrinfo != null)
			jmp123.Jmp123Logger.info(vbrinfo.toString());
	}
...
//		jmp123.Jmp123Logger.infof("\r#%-5d [%-41s] %02d:%05.2f ",framecounter, progress, m, s);
	}
