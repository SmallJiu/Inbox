...
public class ID3Tag {
	...
	private final String[] TEXT_ENCODING = {"GBK", "UTF-16", "UTF-16BE", "UTF-8"};
	...
	public void printTag() {
		//if (lyrics != null)
		//	jmp123.Jmp123Logger.info("\r" + lyrics + "\n");
		if (title != null){}
//			jmp123.Jmp123Logger.info("      [ Title] " + title);
		if (artist != null){}
//			jmp123.Jmp123Logger.info("      [Artist] " + artist);
		if (album != null){}
//			jmp123.Jmp123Logger.info("      [ Album] " + album);
		if (year != null){}
//			jmp123.Jmp123Logger.info("      [  Year] " + year);
	}
	...
	public void parseID3V1(byte[] b, int off) {
		int i;
		if (b.length < 128 || !checkID3V1(b, off))
			return;
	...
	public void parseID3V2(byte[] b, int off, int len) {
		...
		//jmp123.Jmp123Logger.info("ID3 v2." + version);
		while(pos < max_size)
	...
	private int getFrame(byte[] b, int off, int endPos)  {
		...
		//jmp123.Jmp123Logger.info(len+" -------- off = " + off);
		//jmp123.Jmp123Logger.info("ID: " + id + ", id.hashCode()=" + id.hashCode());
		//jmp123.Jmp123Logger.info("text encoding: " + TEXT_ENCODING[enc]);
		//jmp123.Jmp123Logger.info("frame size: " + fsize);
		//if(off>=171)
		//	jmp123.Jmp123Logger.info(len+" -------- off = " + off);
		...
		try {
			...
			/*case 2614438:	//USLT:  歌词
				...
				len -= 4;
				//jmp123.Jmp123Logger.info(new String(b, off, len, TEXT_ENCODING[enc]));
...
//				jmp123.Jmp123Logger.info("[APIC MIME type] " + MIMEtype);
				len -= id_part - off + 1;
				off = id_part + 1;
				int picture_type = b[off] & 0xff;
//				jmp123.Jmp123Logger.info("[APIC Picture type] "+picture_type);
				off++;	//Picture type
				len--;
				for(id_part = off; b[id_part]!= 0 && id_part < endPos; id_part++);
//				jmp123.Jmp123Logger.info("[APIC Description] "
