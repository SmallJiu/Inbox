...
public class PlayBack {
	...
	public void close() {
		...
		if (instream != null)
			instream.close();
		//jmp123.Jmp123Logger.info("jmp123.PlayBack.close() ret.");
	}
	...
	public boolean open(String name, String title) throws IOException {
		...
		if (str.startsWith("http://") || str.startsWith("https://") && str.endsWith(".mp3")) {
			...
			Jmp123Logger.error("Invalid File Format.");
			return false;
		}

		if (!instream.open(name, title))
			return false;
		...
		// 成功解析帧头后初始化音频输出
		if (audio != null
				&& !audio.open(header, id3tag.getArtist()))
			return false;
		...
		if(audio instanceof SpectrumPane) {
			((SpectrumPane) audio).setPlayFileName(title.substring(0, title.lastIndexOf(".")));
		}
	...
	public boolean start(boolean verbose) {
		...
		try {
			...
			// jmp123.Jmp123Logger.info("jmp123.PlayBack.start() interrupt.");
		} finally {
			if (layer != null)
				layer.close();
		}
		// jmp123.Jmp123Logger.info("jmp123.PlayBack.start() ret.");
