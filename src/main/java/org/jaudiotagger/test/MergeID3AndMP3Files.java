...
public class MergeID3AndMP3Files
{
    ...
    public static void main(final String[] args)
    {
        ...
        if (args.length == 0)
        {
            jmp123.Jmp123Logger.error("usage MergeID3AndMP3Files FromDir ToDir mp3File");
            jmp123.Jmp123Logger.error("      You must enter the from dir,outputdir and the mp3file to append");
            System.exit(1);
        }
        else if (args.length != 3)
        {
            jmp123.Jmp123Logger.error("usage MergeID3AndMP3Files FromDir ToDir mp3File");
            jmp123.Jmp123Logger.error("      Only three parameters accepted");
            System.exit(1);
        }
        ...
        {
            jmp123.Jmp123Logger.error("usage MergeID3AndMP3Files FromDir ToDir mp3File");
            jmp123.Jmp123Logger.error("      Directory " + args[1] + " could not be found");
            System.exit(1);
        }
        ...
        {
            jmp123.Jmp123Logger.error("usage MergeID3AndMP3Files FromDir ToDir mp3File");
            jmp123.Jmp123Logger.error("      Mp3File " + args[2] + " could not be found");
            System.exit(1);
        }

        Date start = new Date();
        jmp123.Jmp123Logger.info("Started to merge from:" + rootDir.getPath() + " at " + DateFormat.getTimeInstance().format(start));
        test.scanSingleDir(rootDir, toDir, mp3File);
        Date finish = new Date();
        jmp123.Jmp123Logger.info("Finished to merge from:" + rootDir.getPath() + DateFormat.getTimeInstance().format(finish));
        jmp123.Jmp123Logger.info("Attempted  to merge:" + MergeID3AndMP3Files.count);
        jmp123.Jmp123Logger.info("Successful to merge:" + (MergeID3AndMP3Files.count - MergeID3AndMP3Files.failed));
        jmp123.Jmp123Logger.info("Failed     to merge:" + MergeID3AndMP3Files.failed);
    ...
    private void scanSingleDir(final File fromDir, final File toDir, final File mp3File)
    {
        ...
        for (File audioFile : audioFiles) {
            ...
            try {
                ...
                jmp123.Jmp123Logger.error("Unable to merge record:" + MergeID3AndMP3Files.count + ":" + mp3File.getPath());
