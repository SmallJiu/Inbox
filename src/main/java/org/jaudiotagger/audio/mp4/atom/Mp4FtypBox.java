...
public class Mp4FtypBox extends AbstractMp4Box
{
    ...
    public void processData() throws CannotReadException
    {
        ...
        try
        {
            majorBrand = decoder.decode(dataBuffer.slice().limit(MAJOR_BRAND_LENGTH)).toString();
        }
        ...
        while ((dataBuffer.position() < dataBuffer.limit()) && (dataBuffer.limit() - dataBuffer.position() >= COMPATIBLE_BRAND_LENGTH))
        {
            ...
            try
            {
                String brand = decoder.decode(dataBuffer.slice().limit(COMPATIBLE_BRAND_LENGTH)).toString();
