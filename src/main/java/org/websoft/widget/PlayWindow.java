...
public class PlayWindow extends JFrame
{
	...
	private final SpectrumPane spectrumPane; //频率面板
	private final ListPane listPane;   //歌曲面板
	private final ControllePane controllePanel; //控制面板
	private PlayListThread playlistThread; //播放线程
	private JPopupMenu popupMenu;    //弹出菜单
	private final JMenuItem removeItem;    //删除菜单项
	private final JMenuItem deleteItem;    //直接删除文件
	...
	public void initializeTray() {
		if(SystemTray.isSupported()) {
			...
			try {
				...
				jmp123.Jmp123Logger.info("无法加载托盘!");
			}
		} else {
			jmp123.Jmp123Logger.info("系统不支持托盘!");
		}
	}
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				try {
					...
					jmp123.Jmp123Logger.info(e.toString());
				}
			}
		});
		return button;
